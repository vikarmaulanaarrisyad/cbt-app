...
export default function StudentDashboardPage() {
    ...
    fetch("/api/auth/me")
      ...
      .then((data) => {
        if (data.success && data.data) {
          setStudentSession(data.data);
          if (data.data.user?.tokenUsed) {
            setToken(data.data.user.tokenUsed);
          }
        }
      })
  ...
  const handleToggleAllRules = () => {
    const allChecked = rules.every((r) => r);
    setRules([!allChecked, !allChecked, !allChecked, !allChecked]);
  };
  ...
  const handleNetworkTest = () => {
    ...
    fetch("/favicon.ico", { cache: "no-store", method: "HEAD" })
      ...
      .catch(() => {
        setTimeout(() => {
          setNetLatency(Math.floor(Math.random() * 20) + 12);
          setNetStatus("active");
        }, 500);
      });
  };
  ...
  const handleLogout = async () => {
    const confirmed = await Alert.confirm("Konfirmasi Keluar", "Apakah Anda yakin ingin keluar dari portal siswa?");
    if (confirmed.isConfirmed) {
      try {
        await fetch("/api/auth/logout", { method: "POST" });
        Alert.success("Berhasil Keluar", "Anda telah keluar dari sesi ujian.").then(() => {
          router.push("/login");
        });
      } catch {
        router.push("/login");
  ...
  const handleStartExam = () => {
    ...
    if (!token.trim()) {
      Alert.error("Akses Ditolak", "Token ujian tidak valid atau belum dimasukkan.");
      return;
    }
  ...
  const studentName = studentSession?.user?.name || "Budi Santoso";
  const studentNisn = studentSession?.user?.nisn || "25-3101-0982-014";
  const studentClass = studentSession?.user?.classGroup || "XII MIPA 1";
  const studentLab = studentSession?.user?.labAllocation || "Lab CBT-08";
  const studentJenjang = (studentSession?.user?.educationLevel || "MA").toUpperCase();
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-[0_1px_8px_rgba(0,0,0,0.04)] border-b border-slate-200">
        <div className="h-16 w-full px-4 md:px-8 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 min-w-0 shrink-0">
            ...
            <div className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 border border-blue-100 text-blue-700 text-xs font-bold ml-2">
              <span className="material-symbols-outlined text-[16px]">verified</span>
              <span>Jenjang: {studentJenjang} • CAT Resmi</span>
            </div>
          </div>
          ...
          {/* Connection, Profile & Logout Button */}
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="flex items-center gap-3 pl-1">
              ...
              <div className="w-9 h-9 rounded-full bg-blue-100 border-2 border-blue-200 flex items-center justify-center overflow-hidden shrink-0 text-blue-700 font-bold">
                {studentName.charAt(0)}
              </div>
              <button
                onClick={handleLogout}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-red-50 hover:bg-red-600 text-red-600 hover:text-white transition-all font-bold text-xs border border-red-200 shadow-sm ml-1 cursor-pointer"
                title="Keluar Akun"
              >
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col relative overflow-hidden">
                ...
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100 flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-50 hover:bg-red-600 text-red-600 hover:text-white font-bold text-xs border border-red-200 transition-all cursor-pointer"
                ...
                <div className="flex flex-col items-center text-center pb-6">
                  <div className="relative group">
                    <div className="w-32 h-32 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center relative shadow-sm">
                      <div className="w-20 h-20 rounded-full bg-blue-100 text-blue-600 border border-blue-200 flex items-center justify-center font-black text-3xl">
                        {studentName.charAt(0)}
                      </div>
                  ...
                  <span className="text-xs text-slate-500 font-semibold mt-4 bg-slate-50 px-3 py-1 rounded-full border border-slate-100">
                    Biometrik Wajah Cocok (98.4%)
                  </span>
                </div>
                ...
                <div className="space-y-4 bg-slate-50 rounded-xl p-4 border border-slate-100">
                  <div className="flex flex-col">
                    ...
                    <span className="text-base text-blue-900 font-bold">{studentName}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-200/60">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Jenjang Pendidikan</span>
                      <div className="pt-0.5">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-black border ${
                          ...
                          {studentJenjang === "MI" ? "MI (Ibtidaiyah)" : studentJenjang === "MTS" ? "MTs (Tsanawiyah)" : "MA (Aliyah)"}
                        </span>
                      </div>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kelas / Kelompok</span>
                      <span className="text-xs font-bold text-slate-700">{studentClass}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-200/60">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">NISN / NIK</span>
                      <span className="text-xs font-mono font-bold text-slate-700">{studentNisn}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Ruang Ujian</span>
                      <span className="text-xs font-bold text-blue-700">{studentLab}</span>
                    </div>
                  </div>
                </div>

                <div className="mt-4 flex items-center justify-between text-slate-500 bg-slate-50 px-3 py-2 rounded-lg text-xs font-mono border border-slate-100">
                ...
                <button
                  onClick={handleLogout}
                  className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 hover:bg-red-600 text-red-600 hover:text-white font-bold text-xs border border-red-200 shadow-sm transition-all cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[18px]">logout</span>
                  <span>Keluar Akun Peserta</span>
                </button>
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-blue-600 hover:text-blue-700 bg-blue-50 px-3 py-1 rounded-lg border border-blue-100 transition-colors"
                  >
                    {rules.every((r) => r) ? "Batal Semua" : "Setujui Semua"}
                  </button>
                </div>
                <div className="space-y-3">
                  {[
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="px-3 py-1.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-600 text-[11px]">
                    Pengawas Ruang: <span className="font-bold text-slate-800">{studentLab}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="max-w-xs mx-auto">
                    <input 
                      ...
                      className="w-full text-center tracking-[0.35em] uppercase text-2xl font-black py-4 px-4 rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-800 placeholder:text-slate-300 focus:outline-none focus:border-blue-500 focus:bg-white shadow-inner transition-all font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-slate-100">
                  <button 
                    ...
                    className="flex-1 w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white py-4 px-6 rounded-xl text-lg font-black flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20 hover:shadow-xl hover:-translate-y-0.5 transition-all cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className="w-full sm:w-auto bg-slate-100 hover:bg-slate-200 text-slate-700 py-4 px-6 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors border border-slate-200 cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-[20px]">volume_up</span>
      ...
      <footer className="w-full bg-white py-4 border-t border-slate-200 mt-auto">
        <div className="w-full px-4 md:px-8 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] font-bold text-slate-400">
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">Pengawas: {studentLab}</span>
          </div>
