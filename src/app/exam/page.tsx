...
export default function ExamPage() {
  ...
  // Fetch real questions from database API
  useEffect(() => {
    fetch("/api/questions")
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch((err) => console.error("Error loading questions from DB:", err));
  }, []);
  ...
  const currentDbQuestion = dbQuestions.length > 0
    ? dbQuestions[(activeQuestion - 1) % dbQuestions.length]
    : null;

  const currentOptions: Array<{ id: string; text: string }> = currentDbQuestion?.options && Array.isArray(currentDbQuestion.options) && currentDbQuestion.options.length > 0
    ? currentDbQuestion.options
    : options;

  const currentQuestionText = currentDbQuestion?.text || "Berdasarkan paragraf ke-2, apa simpulan yang paling tepat mengenai korelasi antara alih fungsi lahan sawah untuk panel surya dengan stabilitas harga beras lokal?";
  const currentCategory = currentDbQuestion?.category || "Penalaran Deduktif";
  ...
  return (
    <div className="bg-[#F8FAFC] font-sans text-slate-800 antialiased min-h-screen flex flex-col selection:bg-blue-100 selection:text-blue-900">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex flex-col text-right">
                <span className="text-sm font-semibold text-slate-900 leading-tight">Peserta Ujian</span>
                <span className="text-[10px] text-slate-500 font-medium">Terhubung ke DB Pusat</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-white border-b border-slate-200 sticky top-16 z-40">
        <div className="h-12 w-full max-w-[1600px] mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span className="text-xs text-slate-500 font-medium">Soal <strong className="text-slate-900">{activeQuestion}</strong> dari {Math.max(TOTAL_QUESTIONS, dbQuestions.length)}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
              ...
              {/* Text Passage / Question Text */}
              <div className="my-8">
                <div className={`space-y-4 text-slate-800 leading-relaxed font-medium ${fontClasses[fontSize]}`}>
