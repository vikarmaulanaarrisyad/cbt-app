...
export default function LoginPage() {
  ...
  const handleStudentSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!nisn || !token) {
      Alert.warning("Data Tidak Lengkap", "Mohon isi Nomor Peserta dan Token Sesi Ujian.");
      return;
    }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        Alert.success("Autentikasi Berhasil", "Membuka ruang ujian...").then(() => {
          // Navigate to student exam dashboard (not created yet)
        });
      }, 800);
    }, 1200);
  };

  const handleProctorSubmit = (e: React.FormEvent) => {
    e.preventDefault();
                  ...
                  <form className="flex flex-col gap-5 pl-0 sm:pl-12" onSubmit={handleProctorSubmit}>
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="relative flex items-center rounded-xl border border-[#cbd5e1] bg-white focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/10 transition-all shadow-sm overflow-hidden">
                        ...
                        <input
                          className="w-full bg-transparent px-4 py-3 text-sm font-mono text-slate-900 outline-none font-bold"
                          id="proctor-nip"
                          required
                          type="text"
                          defaultValue="198402122008011004"
                        />
                      </div>
                    </div>
                    ...
                    <div className="pt-4">
                      <button
                        className="w-full bg-[#0a0f1c] hover:bg-slate-800 text-white font-bold text-base py-4 px-6 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 group"
                        type="submit"
                      >
